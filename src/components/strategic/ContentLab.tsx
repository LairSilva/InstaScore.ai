...
export const ContentLab: React.FC<ContentLabProps> = ({ dna, onOpenPaywall, isPro = true }) => {
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('/api/strategic/content-lab', {
        method: 'POST',
      ...
      if (data.success && data.item) {
        ...
        alert(data.message || 'Não foi possível gerar a peça estratégica.');
      }
    } catch (err) {
      console.error('Content Lab error:', err);
      alert('Erro de conexão ao gerar conteúdo estratégico.');
    } finally {
      setIsLoading(false);
    }
  };
