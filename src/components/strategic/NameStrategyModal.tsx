...
export const NameStrategyModal: React.FC<NameStrategyModalProps> = ({
  ...
  const handleGenerateNames = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/strategic/naming', {
        method: 'POST',
      ...
      if (data.success && Array.isArray(data.recommendations)) {
        ...
        alert(data.message || 'Erro ao gerar estratégias de nomes.');
      }
    } catch (err) {
      console.error('Name Strategy fetch error:', err);
      alert('Erro de conexão ao gerar estratégias de nomes.');
    } finally {
      setIsLoading(false);
    }
  };
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-base font-bold text-white flex items-center gap-2">
                          {item.display_name}
                          <span className="text-xs font-mono text-gray-400">{item.handle}</span>
                        </h4>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => handleCopy(`${item.display_name} | ${item.handle}`, `name_${idx}`)}
                        className="text-xs bg-white/10 hover:bg-white/20 text-gray-200 px-2.5 py-1.5 rounded-lg flex items-center gap-1 transition-colors"
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          onApplyName(item.display_name, item.handle);
                          onClose();
                        }}
                        className="text-xs bg-cyan-600 hover:bg-cyan-500 text-white font-semibold px-3 py-1.5 rounded-lg flex items-center gap-1 transition-colors"
                      >
                  ...
                  <p className="text-xs text-gray-300 bg-white/5 p-2.5 rounded-lg border border-white/5">
                    <strong>Lógica Estratégica:</strong> {item.strategic_logic}
                  </p>
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center text-[10px] pt-1">
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      ...
                      <strong className="text-cyan-300">{item.memorability_score}/100</strong>
                    </div>
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      <span className="text-gray-400 block">Clareza de Nicho</span>
                      <strong className="text-cyan-300">{item.clarity_score}/100</strong>
                    </div>
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      <span className="text-gray-400 block">Diferenciação</span>
                      <strong className="text-cyan-300">{item.differentiation_score}/100</strong>
                    </div>
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      <span className="text-gray-400 block">Potencial de Marca</span>
                      <strong className="text-cyan-300">{item.brand_potential_score}/100</strong>
                    </div>
                  </div>
                </div>
              ))}
