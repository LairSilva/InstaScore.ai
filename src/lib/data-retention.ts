...
export async function exportUserData(userId: string): Promise<{
  ...
  if (adminDb) {
    try {
      ...
      console.warn('[DataRetention] Firestore query warning during export:', err);
    }
  } else {
    ...
    const fbCol = getLocalCollection('feedback');
    for (const [id, doc] of fbCol.entries()) {
      if (doc.userId === userId) resultData.feedback.push({ id, ...doc });
    }
  }
...
export async function deleteUserData(userId: string): Promise<{
  ...
  if (adminDb) {
    ...
    // In-memory / local deletion fallback
    ...
    deletedCounts.diagnoses = deleteFromMap('diagnoses');
    deletedCounts.startProjects = deleteFromMap('start_projects');
    deletedCounts.digitalTwins = deleteFromMap('digital_twins');
    deletedCounts.profileDna = deleteFromMap('profile_dna');
    deletedCounts.performanceRecords = deleteFromMap('performance_memory');
    deletedCounts.feedback = deleteFromMap('feedback');
    deletedCounts.usage = deleteFromMap('usage');
    deletedCounts.subscription = deleteFromMap('subscriptions');
  }
  ...
  if (adminDb) {
    for (const colName of collectionsScanned) {
      try {
        ...
        console.warn(`[DataRetention] Expired cleanup query warning on ${colName}:`, err);
      }
    }
  }
